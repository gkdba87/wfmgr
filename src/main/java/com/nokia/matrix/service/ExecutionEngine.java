...
public class ExecutionEngine {
	 ...
	 * @throws Exception
	 */
	private boolean executeTask(Long workflowId, String taskName) throws WorkFlowExecuteException, Exception {

		// Check in DB if task is already executed.
		if(isTaskAlreadyExecuted(workflowId, taskName)) {
			return true;
		}
		BuildInfo buildInfo =  jenkinsHelper.triggerBuild(taskName, workflowId);
	 ...
	 * @throws Exception
	 */
	public void executeWorkFlow(Long workflowId) throws WorkFlowExecuteException, Exception {
		boolean end = false;
		...
		while (!end) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (taskToExecute != null) {
					...
					if (executeTask(workflowId, taskToExecute)) {
						mapTaskStatus.put(taskToExecute, "1");
					} else {
						mapTaskStatus.put(taskToExecute, "0");
						if (!isNextFlowElementExclusiveGw(taskToExecute)) {
	...
	public boolean isTaskAlreadyExecuted(Long workflowId, String taskName) {

		boolean isExecuted = true;
		try {
			Task task = taskRepo.findTaskByName(workflowId, taskName);
			if(TaskStatus.FAILED.name().equals(task.getStatus()) || TaskStatus.NOT_STARTED.name().equals(task.getStatus())) {
				isExecuted = false;
			}
		} catch (NullPointerException exe) {
			log.error("No task found with taskName: " + taskName + "with exception" + exe);
